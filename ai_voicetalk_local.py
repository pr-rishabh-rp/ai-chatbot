if __name__ == '__main__':
    ...
    recorder = AudioToTextRecorder(
        ...
        language="en", 
        spinner=False,
        silero_sensitivity=0.3,           # Lower = less sensitive to pauses (default: 0.5)
        post_speech_silence_duration=1.0, # Grace period: wait 1 second after speech ends
        min_length_of_recording=0.5,      # Minimum recording length in seconds
        min_gap_between_recordings=0      # No gap required between recordings
    )


    print()
    while True:
        voice_number = input(f"Select voice (1-6): ")
